...
try:
    ...
    print("failed=True msg='`requests` library required for this script'")
    sys.exit(1)
...
command line argument (--api-token), or environment variable (ONLINE_NET_API_TOKEN)''')
    ...
    def read_cli_args(self):
        ...
        parser.add_argument('--list', action='store_true',
                            help='List all Online.net servers as Ansible inventory (default: True)')
        ...
        parser.add_argument('--servers', nargs='*', default=[], action='store', help='Specify servers to get info for')
        parser.add_argument('--clusters', nargs='*', default=[], action='store',
                            help='Specify cluster to filter info by')
        ...
        parser.add_argument('--refresh-cache', '-r', action='store_true', default=False,
                            help='Force refresh of cache by making API requests to Online.net (default: False - use cache files)')

        parser.add_argument('--env', '-e', action='store_true',
                            help='Display ONLINE_NET_API_URI and ONLINE_NET_API_TOKEN')
        ...
        if args.cache_max_age:
            self.cache_max_age = int(args.cache_max_age)
    ...
    def load_from_online_net(self):
        ...
        if self.args.servers:
            for server_id in self.args.servers:
                servers.append(self.api('server/' + server_id))
        else:
            servers_uris = self.api()
            for server_uri in servers_uris:
                server_info = self.api('server/' + server_uri.rsplit('/', 1)[1])
                if self.args.clusters:
                    if not any(cl in server_info['hostname'] for cl in self.args.clusters):
                        continue
    ...
    def build_index(self, data, index_key):
        ...
        for idx in enumerate(data):
            if 'network.ip' == index_key:
                ...
                if 'name' not in idx[1]['os']:
                    idx[1]['os'] = {'name': 'none'}
                key = idx[1]['os']['name'].replace(' ', '_')
    ...
    def build_inventory(self):
        ...
        for server in self.data:
            ...
            self.push(self.inventory, 'os_' + server['os']['name'].replace(' ', '_'), dest)
            for i in range(len(server['disks'])):
                for k in server['disks'][i]:
                    if k == '$ref':
                        server['disks'][i] = self.api(server['disks'][i][k].replace('/api/v1/', ''))
    ...
    def api(self, command='server'):
        resp = requests.get(self.api_uri + command,
                            headers={'Content-Type': 'application/json', 'Authorization': 'Bearer ' + self.api_token})
