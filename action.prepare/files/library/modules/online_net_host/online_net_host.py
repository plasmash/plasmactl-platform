    ...
    def state(self, state):
        if state == 'on':
            ...
            if 'rescue-' in self.boot_mode:
                self.changed = True
                return self.api('server/boot/rescue/' + str(self.id), dict(image=self.boot_mode.replace('rescue-', '', 1)))
            elif 'exit-rescue' in self.boot_mode:
                self.changed = True
                return self.api('server/boot/normal/' + str(self.id), dict(reason='Exit rescue mode by Ansible plugin'))
            else:
                self.changed = True
                return self.api('server/reboot/' + str(self.id), dict(reason='Rebooted by Ansible plugin'))
        else:
            return False
...
def core(module):
    ...
    if not server:
        ...
        if hostname:
            output['hostname'] = server.name(hostname)

        if rpn_groups:
            output['rpn_groups'] = server.rpn_groups(rpn_groups)

        if rescue_images:
            output['rescue_images'] = server.rescue_images()

        if bmc:
            output['bmc'] = server._bmc(bmc)

        if bmc_close:
            output['bmc_close'] = server.bmc_close(bmc_close)

        if boot_mode:
            server.boot_mode = boot_mode

        if state:
            output['state'] = server.state(state)

        module.exit_json(changed=server.has_changed(), server=server.to_json(), output=output)
