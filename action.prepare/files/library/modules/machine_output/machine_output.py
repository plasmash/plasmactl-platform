...
class CallbackModule(CallbackModule_default):
    ...
    def _get_task_display_name(self, task):
        ...
        if ":" not in task_display_name:
            if task._role!=None:
                task_display_name = task._role.get_name() + " : " + task_display_name
        task_display_name = time.strftime("%H:%M:%S - ") + task_display_name
    ...
    def _process_result_output(self, result, msg):
        ...
        if self.delegated_vars:
            ...
            task_result = "%s %s -> %s" % (msg, task_host, task_delegate_host)

        if result._result.get('msg') and result._result.get('msg') != "All items completed":
            task_result += "\n" + to_text(result._result.get('msg'))
    ...
    def v2_runner_on_skipped(self, result, ignore_errors=False):
        if self.get_option('display_skipped_hosts'):
            ...
            task_result = self._process_result_output(result, "⨯")
